...
public interface CacheProviders {
    ...
    rx.Observable<Reply<List<DataResults>>> getHomeTypes(rx.Observable observable, DynamicKey username, EvictDynamicKey evictDynamicKey);
    //缓存时间 1天
    @LifeCache(duration = 7, timeUnit = TimeUnit.DAYS)
    rx.Observable<Reply<List<ZhiHuResults>>> getZhiTypes(rx.Observable observable, DynamicKey userName, EvictDynamicKey evictDynamicKey);

    //缓存时间 1天
    @LifeCache(duration = 7, timeUnit = TimeUnit.DAYS)
    rx.Observable<Reply<List<HuaResults>>> getHuaTypes(rx.Observable observable, DynamicKey userName, EvictDynamicKey evictDynamicKey);
}
